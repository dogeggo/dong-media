...
- **交互式演员作品查看器**：播放页面内联显示演员作品，TMDB 演员搜索缓存 6 小时、作品和详情缓存 12 至 24 小时
...
- **智能缓存机制**：标准化弹幕结果在服务端共享缓存 24 小时，浏览器只保留 Query 会话状态
...
- **Netflix风格HeroBanner**：首页带预告片自动播放和背景图的Netflix风格横幅，支持backdrop占位符；豆瓣详情与短时预告片地址分离缓存，403/404 自动精确失效并重新解析
  - **高清背景图**：背景图始终使用高清版本
...
- **豆瓣详情增强**：评分、演职人员、首播日期、时长、制作信息完整展示，海报代理防403错误；详情按统一策略缓存 7 天，预告片地址按 4 小时策略独立管理
...
- **日历缓存迁移**：发布日历以完整数据集原子缓存 8 小时，筛选和分页在缓存读取后执行，不再维护浏览器业务副本
- **收藏数据库存储**：全局收藏数据存储到数据库，支持跨设备同步和分类管理
- **缓存优化**：所有业务策略集中注册，支持有界 L1、Redis/Kvrocks L2、single-flight、generation 失效、TTL 抖动、负缓存和 stale-if-error
- **存储模式增强**：Kvrocks/Redis 完整支持；共享缓存不可用时自动 fail-open，本地存储模式下用户数据仍作为浏览器主存储
- **图片/视频代理优化**：静态媒体统一一周缓存，磁盘层具有 URL 哈希键、原子写、TTL、容量水位、LRU、条件请求与 Range 支持
...
- ✅ 有界 L1 内存缓存与 Redis/Kvrocks 故障降级
- ✅ 用户注册系统（可配置开关）
...
- 按 namespace 查看 L1、L2 与媒体磁盘的条目、空间、命中率、错误和 single-flight 合并次数
- 通过 generation 常数时间失效业务缓存，清理过期/损坏磁盘对象；不会删除用户偏好或 localStorage 模式下的用户主数据
- Docker、Cloudflare Worker/R2 与运维参数见 [缓存部署与运维](./CACHE_OPERATIONS.md)
